...
public class CalendarTool {
    ...
    public CalendarTool() {
        ...
        setGregorianDate(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1,
    ...
    public CalendarTool(CharSequence time) {
        String str = String.valueOf(time);
        String[] out = str.split("/");
        setGregorianDate(Integer.parseInt(out[0]), Integer.parseInt(out[1]), Integer.parseInt(out[2]));
    ...
    public String getWeekDayStr() {
        String weekDayStr[] = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday",
                "Sunday"};
    ...
    private void IranianCalendar() {
        ...
        int Breaks[] = {-61, 9, 38, 199, 426, 686, 756, 818, 1111, 1181, 1210, 1635, 2060, 2097, 2192, 2262,
                2324, 2394, 2456, 3178};
    ...
    private int julianDateToJDN(int year, int month, int day) {
        return (year + (month - 8) / 6 + 100100) * 1461 / 4 + (153 * ((month + 9) % 12) + 2) / 5 + day
                - 34840408;
    }
    ...
    private int gregorianDateToJDN(int year, int month, int day) {
        int jdn = (year + (month - 8) / 6 + 100100) * 1461 / 4 + (153 * ((month + 9) % 12) + 2) / 5 + day
                - 34840408;
    ...
    final static char[] persianNumbers = {'۰', '۱', '۲', '۳', '۴', '۵', '۶', '۷', '۸', '۹'};

    public static String convertAllNumbersToPersian(String s) {
        String priceString = String.valueOf(s);
        for (int i = 0; i < priceString.length(); i++) {
            char currentChar = priceString.charAt(i);
            int currentNumberAscii = (int) currentChar;

            //check if number ascii is an English number
            if (currentNumberAscii >= 48 && currentNumberAscii <= 57) {
                int currentNumber = Integer.valueOf(priceString.substring(i, i + 1));
                priceString = priceString.replace(currentChar, persianNumbers[currentNumber]);
            }
        }
        return priceString;
    }
